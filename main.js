...
}, 60 * 1000) // menyimpan database setiap menit
...
// Auto delete file on tmp
// let files = fs.readdirSync('./tmp').map(file => path.join(__dirname, './tmp', file)).filter(file => fs.statSync(file).isFile())
