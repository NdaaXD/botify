...
`.trim()
     ...
     * @param {Object} options
     */
    async sendContactArray(jid, array, arrayEmail, quoted, options) {
      let list = []
      let e = -1
      for (let i of array.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net')) {
        let name = db.data.users[i] ? db.data.users[i].name : this.getName(i)
        list.push({
          "displayName": name,
          "vcard": `BEGIN:VCARD
VERSION:3.0
...
item2.EMAIL;type=INTERNET:${arrayEmail[e + 1]}
item2.X-ABLabel: Email
item3.URL: https://melcanz.com/
 ...
 */
    ...
    */
     ...
     * ms to date
     * @param {String} ms
     * @param {Boolean} onlyday
     * @param {Boolean} onlyhour
     * @param {Boolean} onlyminute
     * @param {Boolean} onlysecond
     */
    msToDate(ms, onlyday, onlyhour, onlyminute, onlysecond) {
      ...
      if (onlyday) return days + ' Hari'
      if (onlyhour) return hours + ' Jam'
      if (onlyminute) return minutes + ' Menit'
      if (onlysecond) return sec + ' Detik'
      if (onlyday && onlyhour) return days + ' Hari,' + hours + ' Jam'
      if (onlyday && onlyhour && sec && onlysecond) return days + ' Hari,' + hours + ' Jam, ' + minutes + ' Menit, ' + sec + ' Detik'
